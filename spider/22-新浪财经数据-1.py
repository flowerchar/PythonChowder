...
for j in tqdm(range(1,153)):
    base_url = f"https://www.china-cba.net/Index/lists/catid/14/p/{j}.html"
    driver.get(base_url)
    lis = driver.find_elements_by_xpath("//li//a")
    dates = driver.find_elements_by_xpath("//li")
    for li, i in zip(lis, dates):
        resp = requests.get(li.get_attribute('href'))
        html = etree.HTML(resp.text)
        article = ''.join(html.xpath('//*[@id="neirong"]//text()'))
        year = i.text.split('\n')[1][:4:]
        insert(year)
for i in years:
